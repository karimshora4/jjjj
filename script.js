...
let accountForms = document.querySelectorAll(".accountForm");

for (let i = 0; i < accountForms.length; i++) {
  accountForms[i].addEventListener("submit", function (e) {
    e.preventDefault();
    let message = accountForms[i].querySelector(".formMsg");

    if (message != null) {
      message.innerText = accountForms[i].dataset.msg;
    } else {
      alert("Success");
    }
  });
